// bff/src/db/init.js
// Phase 1 迁移：sql.js → better-sqlite3
// 关键变化：
//   1. init() 同步（better-sqlite3 是同步 API）
//   2. 保留 globalThis（vitest 4.x 跨 worker 不复用模块实例，必须全局共享 db）
//   3. 删 saveDB()（better-sqlite3 自动持久化）
//   4. 保留旧 wrapper API（让 routes 不用改）
//   5. wrapper.run() 真传 changes（不强制 || 1，保护 P0-NEW-2 乐观锁）
...
if (!globalThis.__ERP_DB_STATE__) {
  globalThis.__ERP_DB_STATE__ = { db: null, dbPath: null, initPromise: null };
}
const STATE = globalThis.__ERP_DB_STATE__;

function init() {
  // 同步初始化（idempotent：re-init 直接返回）
  ...
  STATE.db = new Database(STATE.dbPath);
  ...
  try {
    ...
    if (ftsCheck) {
      console.log('✅ FTS5 candidates_fts 虚拟表已就绪（better-sqlite3 内置 FTS5）');
    } else {
...
function prepare(sql) {
  ...
  return {
    ...
    run(...params) {
      ...
      // 真传 changes（不 || 1，保护 P0-NEW-2 乐观锁 / UPDATE WHERE 不命中 → changes=0）
...
function createTables() {
  ...
  // better-sqlite3 内置 FTS5；保留 try/catch 兼容极端降级场景。
  try {
    exec(`
    ...
    exec(`INSERT OR IGNORE INTO candidates_fts(candidate_id, name, phone, email, current_company)
...
function seedUsersIfNeeded() {
  ...
  const cnt = prepare('SELECT COUNT(*) as cnt FROM users').get().cnt;
  ...
  if (isSeedRun) {
    exec("DELETE FROM users WHERE username IN ('admin', 'demo')");
  }
  ...
  const taskCnt = prepare('SELECT COUNT(*) as cnt FROM tasks').get().cnt;
  if (taskCnt === 0) {
    seedBusinessData();
  }
}
...
function seedJobs() {
  const jobCnt = prepare('SELECT COUNT(*) as cnt FROM jobs').get().cnt;
  if (jobCnt > 0) return;

  const adminId = prepare("SELECT id FROM users WHERE username = 'admin'").get()?.id;
  if (!adminId) return;

  const insertJob = prepare(`
    INSERT INTO jobs
...
function seedCandidates() {
  const candCnt = prepare('SELECT COUNT(*) as cnt FROM candidates').get().cnt;
  if (candCnt > 0) return;

  const adminId = prepare("SELECT id FROM users WHERE username = 'admin'").get()?.id;
  if (!adminId) return;

  const insertCand = prepare(`
...
function getDb() {
  ...
  // 暴露 wrapper（routes 用 prepare/exec）
  return { prepare, exec };
}
...
module.exports = { init, getDb, isReady, get db() { return { prepare, exec }; } };
